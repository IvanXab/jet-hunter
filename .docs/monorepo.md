...
Каждый цвет задан парой через `light-dark(<светлый>, <тёмный>)`: тёмные значения
взяты из дизайна «Console UI Kit v2». Какое из двух применится, решает
`color-scheme` на `:root`, который выставляет `theme.css`:

- по умолчанию `light dark` — тема следует системной настройке
  (`prefers-color-scheme`);
- `<html data-theme="light">` или `<html data-theme="dark">` — тема
  зафиксирована, например для переключателя в интерфейсе.

Во фронтенде это делает хук `useTheme` (`apps/frontend/src/theme/`). Пока
пользователь ничего не выбрал, приложение следует теме ОС и реагирует на её
смену на лету. Переключатель «Тёмная тема» внизу сайдбара меняет тему, а выбор
сохраняется в `localStorage` (ключ `jet-hunter:theme`). Если выбранная тема
совпадает с темой ОС, выбор сбрасывается, и приложение снова следует за
системой.

Классы в компонентах от темы не зависят, `dark:`-варианты не нужны. Добавляя
цвет, задавайте его сразу парой в `light-dark()`. Переопределять тёмный цвет
тоже нужно парой: `--color-accent: light-dark(#4b6cff, #7d93ff)`.

`light-dark()` работает в Chrome 123+, Safari 17.5+ и Firefox 120+. В браузерах
старше этих версий цвета не применятся.
...
Console»). Тёмная тема — из «Console UI Kit v2», подробнее в разделе
«Светлая и тёмная тема».
